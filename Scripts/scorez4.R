...
#data partition
set.seed(101)
labelz4$typeZ4 <- as.factor(labelz4$typeZ4)
sample = sample.split(labelz4, SplitRatio = .80)
train = subset(labelz4, sample == TRUE)
test  = subset(labelz4, sample == FALSE)
...
# plot tree 
rpart.plot(fit)
fancyRpartPlot(fit, palettes = c("Greys", "Oranges"), main="Z4")

#prediction & model evaluation
pred <- predict(fit, test, type = 'class')
#type 1, type 2 error, cross validation
table(pred, test$typeZ4) 

mean(pred == test$typeZ4)
